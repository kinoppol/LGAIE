...
if ($existing) {
    db_run('UPDATE course_certificates SET enabled = ?, grade_json = ?, background_style = ? WHERE course_id = ?',
        [$enabled, $json, $bg_style, $course_id]);
} else {
    db_run('INSERT INTO course_certificates (course_id, enabled, grade_json, background_style) VALUES (?,?,?,?)',
        [$course_id, $enabled, $json, $bg_style]);
}
